...
/*   Updated: 2022/04/20 21:39:38 by dzivanov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::signForm(AForm &form) const
{
	...
	{
		form.beSigned(*this);
		std::cout << YELLOW << _name << DEFAULT << " signs " << GREEN << form.getName() << DEFAULT << std::endl;
	}
	catch(const std::exception& error)
	{
		std::cout << RED << _name << " couldn't sign " << GREEN << form.getName() << DEFAULT << " because " << YELLOW << error.what() << DEFAULT << std::endl;
	}
}
...
void Bureaucrat::executeForm(const AForm &form) const
{
	...
	{
		form.execute(*this);
		std::cout << YELLOW << _name << DEFAULT << " executes " << GREEN << form.getName() << DEFAULT << std::endl;
	}
	catch (const std::exception &error)
	{
		std::cout << YELLOW << _name << DEFAULT << " cannot execute " <<  GREEN << form.getName() << DEFAULT << " because " << CYION << error.what() << DEFAULT << std::endl;
	}
}
